...
//
// When [MetricSegmentationMatrix.IsKnown] reports true, exactly one variant pointer is
...
// unknown variant's JSON is available from [MetricSegmentationMatrix.Raw].
...
// knows about, i.e. one of the MetricSegmentationMatrix* constants. It is false for a
// variant added to the API later, whose payload is then only available from
// [MetricSegmentationMatrix.Raw].
func (u MetricSegmentationMatrix) IsKnown() bool {
	switch u.Type {
	case MetricSegmentationMatrixSingle, MetricSegmentationMatrixDouble, MetricSegmentationMatrixLinked:
		return true
	}
	return false
}
...
// decoded (see [MetricSegmentationMatrix.IsKnown]). It is nil for a known variant, and
// for a value that was not decoded from JSON. The returned slice is a copy.
func (u MetricSegmentationMatrix) Raw() json.RawMessage {
