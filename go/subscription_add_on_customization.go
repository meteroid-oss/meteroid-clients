...
//
// When [SubscriptionAddOnCustomization.IsKnown] reports true, exactly one variant pointer is
...
// unknown variant's JSON is available from [SubscriptionAddOnCustomization.Raw].
...
// knows about, i.e. one of the SubscriptionAddOnCustomization* constants. It is false for a
// variant added to the API later, whose payload is then only available from
// [SubscriptionAddOnCustomization.Raw].
func (u SubscriptionAddOnCustomization) IsKnown() bool {
	switch u.Type {
	case SubscriptionAddOnCustomizationPriceOverride, SubscriptionAddOnCustomizationParameterization:
		return true
	}
	return false
}
...
// decoded (see [SubscriptionAddOnCustomization.IsKnown]). It is nil for a known variant, and
// for a value that was not decoded from JSON. The returned slice is a copy.
func (u SubscriptionAddOnCustomization) Raw() json.RawMessage {
