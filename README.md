...
Every SDK sends the API version it was generated against as the `Meteroid-Version` header, so a
server that no longer serves that snapshot answers with a clear error instead of a changed
payload shape.

For each sync the workflow:

1. runs `scripts/spec_sync.py`, which copies the spec, bumps `.version`, pins the API version in
   each SDK's API version file (`rust/src/api_version.rs`, `ApiVersion.java`, `apiVersion.ts`,
   `_api_version.py`, `go/api_version.go`), drafts the `CHANGELOG.md` entry from
...
The existing CI (Codegen consistency plus the per-language lint and test workflows) is the gate.
Once `.version` lands on `main`, `Mega Releaser` runs and the SDK releases follow.
...
requiring the language checks above, and an `SDK_SYNC_TOKEN` secret in `enterprise` with
